...
export function useTaskManager(apps: ManagedAppDefinition[], runnableApps: ManagedAppDefinition[]) {
  ...
  async function refreshCloudServers(): Promise<CloudServerRow[]> {
    try {
      const [result, xpFarmHosts, hwgwHosts] = await Promise.all([
        fetchCloudList(callAction),
        readXpFarmHosts(ns),
        readHwgwHosts(ns),
      ])
      const dedicated = new Set([...xpFarmHosts, ...hwgwHosts])
  ...
  function appAvailable(app: ManagedAppDefinition): boolean {
    return checkIsAvailable(app.isAvailable, { homeRam, daemonTier, darkscapeNavigator: false, ...resetInfo })
  }
