 ...
 *
 * Extracted from `daemons/xp-farm.daemon.ts` (its original home, still its
 * only caller for the XP Farm feature itself) so `daemons/money-farm.daemon.ts`
 * can reuse the identical grow-prep logic without duplicating it — behavior
 * unchanged from the original.
 */
export function splitGrowWeakenThreads(ns: NS, totalThreads: number): { growThreads: number, weakenThreads: number } {
  if (totalThreads <= 1)
