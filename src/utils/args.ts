...
export function parseArgs<T extends readonly BitburnerFlagSpec[]>(
    ...
    for (const item of customSchema) {
        ...
        let typeLabel = "";
        if (typeof item.defaultValue === "string") typeLabel = " <string>";
        if (typeof item.defaultValue === "number") typeLabel = " <number>";
        if (Array.isArray(item.defaultValue)) typeLabel = " <array>";
