...
import type { TradeSignal } from './lib/trader/signal.momentum'
import { parseStockStatsLog, recordStockTick, STOCK_STATS_LOG_FILE } from './lib/stock-stats/state-file'
import { getSignal, PriceWindow, WINDOW_TICKS } from './lib/trader/signal.momentum'
...
/**
 * FROZEN FALLBACK - a byte-for-byte snapshot of stock-trader.app.ts as it
 * stood right before that script switched to an up-tick-frequency momentum
 * estimator (see lib/trader/signal.momentum.ts's own header). The only
 * difference from stock-trader.app.ts is the import source for
 * getSignal/PriceWindow/WINDOW_TICKS/TradeSignal - everything else,
 * including every constant and the whole trading loop, is identical. Exists
 * so the window-endpoint-difference momentum approach (already tested
 * across several live sessions) stays one command away - `run
 * stock-trader.momentum.app.js` - without needing git archaeology to
 * reconstruct it, if the new estimator turns out worse in practice.
 * Deliberately NOT kept in sync with future edits to stock-trader.app.ts.
 *
 ...
 * 4S Market Data TIX API is owned) or trader/signal.momentum.ts's
 * rolling-window momentum fallback otherwise. Every
 * ns.stock.getPurchaseCost/getSaleGain call already bakes in
 * commission/spread/price-impact, so there's no separate "$100k penalty"
 * constant anywhere in here - it's just always priced in by those two
 * functions.
 *
 * Live by default: `run stock-trader.momentum.app.js` executes real trades.
 * Pass --dry-run to only log what it would have done.
 */
 ...
 * see trader/signal.momentum.ts's WINDOW_TICKS comment), not a single
 * tick: spread alone runs ~1.2% (this project's stock-stats.txt sample)
 * and per-tick volatility sits in the same ~0.5-2.5% range, so comparing a
 * round-trip cost paid once against a single tick's expected move made the
 * edge check nearly unsatisfiable regardless of signal quality.
 *
